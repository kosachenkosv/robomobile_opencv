...
g++ -ggdb `pkg-config --cflags opencv`  binary.cpp mySerial.cpp -o binary `pkg-config --libs opencv`  


g++ `pkg-config --cflags opencv` -o binarypc binarypc.cpp `pkg-config --libs opencv`
...
int main(int argc, char* argv[])
{
		...
		// получаем любую подключённую камеру на RPI
		//CvCapture* capture = cvCreateCameraCapture(CV_CAP_ANY);

		// на моем ноуте внешняя USB-вебка
		CvCapture* capture = cvCreateCameraCapture(CV_CAP_ANY);
